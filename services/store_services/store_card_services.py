...
from db.models import Store, User, UserCard, StoreRecord
...
async def query_store_service(
	...
	card_rarity: Optional[CardRarity] = None,
	...
	if card_rarity is not None:
		query &= Q(card__rarity=card_rarity)
...
async def list_card_service(
	...
	seller_card = await UserCard.filter(
		user_id=user_id,
		card__name=card_to_list.name,
		number__gte=card_to_list.number,
	).select_related('card').select_for_update().first()
	if not seller_card:
	...
	# 3.商店中有该卖家上架的该卡牌则增加数量并改价，没有则新建
	store_card = await Store.filter(
		card_id=seller_card.card.id,
		owner_id=user_id
...
async def buy_card_service(
	...
	store_card = await Store.filter(
		...
		number__gte=card_to_buy.number,
	...
	return need_byte, store_card.card.name
